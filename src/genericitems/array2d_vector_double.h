...
template <> class GenericItemContainer<Array2D<std::vector<double>>> : public GenericItem
{
    ...
    {
        return std::make_shared<GenericItemContainer<Array2D<std::vector<double>>>>(name, flags);
    }

    public:
    // Return class name contained in item
    std::string_view itemClassName() override { return "Array2D<std::vector<double>>"; }
