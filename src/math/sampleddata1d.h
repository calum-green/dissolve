...
class SampledData1D : public PlottableData, public ListItem<SampledData1D>, public ObjectStore<SampledData1D>, public GenericItemBase
{
    public:
    SampledData1D();
    virtual ~SampledData1D();
    ...
    void initialise(const SampledData1D &source);
    // Copy arrays from supplied object
    void copyArrays(const SampledData1D &source);
    // Zero values array
    void zero();
    // Return data version
    int version() const;
     ...
     */
    public:
    void operator=(const SampledData1D &source);
    void operator+=(const SampledData1D &source);
